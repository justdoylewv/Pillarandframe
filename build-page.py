...
h+=['<section class="section wrap sameness" id="why"><div><p class="eyebrow">THE SAMENESS PROBLEM</p><h2>Quality. Reliable.<br>We go the extra mile.</h2></div><div>'+p('Your customers have heard it all before. Every contractor says it, so none of it lands. Good companies end up sounding like the ones cutting corners.')+p("The difference is out on your projects. It is in the call your PM made when the drawings were wrong. The problem you found before it became the owner's problem. The work you stood behind after the invoice was paid.")+p('We bring that into focus, and put it where it does the work.')+'</div></section>']
...
core={
 'written':('The core three',money('core_written'),'The film, the written case study and the social set. Everything one job produces, out of one day on site.',
   ['Hero case study film, 2\u20134 min','Written case study, 800\u20131,200 words','Social cut set, 5\u201310 verticals at 15\u201345s']),
 'video':('The core two',money('core_video'),'The film and the social set, without the written case study. Same day on site, same footage.',
   ['Hero case study film, 2\u20134 min','Social cut set, 5\u201310 verticals at 15\u201345s']),
}
addon_films=[
...
for key,(name,price,desc,features) in core.items():
 h+=['<div class="pricing-grid solo" id="panel-'+key+'" role="tabpanel" aria-labelledby="tab-'+key+'"'+('' if key=='written' else ' hidden')+'><article class="price-card featured"><h3>'+name+'</h3><p class="price">'+price+'</p>'+p(desc)+'<ul class="feature-list">'+''.join('<li>'+f+'<span aria-label="Included">\u2713</span></li>' for f in features)+'</ul>'+cta('Book an intro call')+'</article></div>']
...
h+=['<section class="section soft"><div class="wrap">'+heading('CLIENTS','In their words.')+'<div class="three-grid">'+''.join(quote_card(i) for i in range(1,4))+'</div></div></section>']
terms=[('Thirty days or you do not pay.','Every deliverable in your hands thirty days after we leave the site. A day late and the project is free and you keep everything.'),('We work around your job.','Up to a full day on site, on your schedule. The work does not slow down.'),('If the footage is not right, we come back on our dime.','Our crews are local, so there are never travel costs. Not for the shoot and not if we need to come back.')]
